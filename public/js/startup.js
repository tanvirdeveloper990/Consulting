...
function createCalenders(calendar, minDate, maxDate, onChange) {
...
function createCalendersWithRang(calendar, minDate, maxDate, calenderType, rang, onChange) {
...
function createFromToCalenders(fromCalendar, toCalendar, minDate, maxDate, calenderType, rang, onChange) {
    createCalendersWithRang(fromCalendar, minDate, maxDate, rang, function (dates) {
        if (onChange)
            onChange(dates);
        else
            $(this).trigger("change").parsley().reset();
        $(toCalendar).calendarsPicker('option', 'minDate', dates[0] || null);
    });
    createCalendersWithRang(toCalendar, minDate, maxDate, rang, function (dates) {
        if (onChange)
            onChange(dates);
        else
            $(this).trigger("change").parsley().reset();
        $(fromCalendar).calendarsPicker('option', 'maxDate', dates[0] || null);
    });
}